...
/*   Updated: 2021/03/18 18:00:43 by jackjoo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			t_bonus *bonus)
{
	...
	while (++y_head < bonus->mini.y_tail)
	{
		...
		{
			if (cur == x || cur == end - 1 ||\
				y_head == bonus->mini.y_tail - 1 ||\
				y_head == bonus->mini.y_head)
...
static void	draw_map(t_info *info, t_bonus *bonus)
{
	...
	while (++cnt_h < info->map.h)
	{
		...
		{
			bonus->mini.y_head = (bonus->mini.h / info->map.h) * \
								tab_h;
			bonus->mini.y_tail = bonus->mini.y_head + \
								bonus->mini.h / info->map.h;
			bonus->mini.x_head = (bonus->mini.w / info->map.w) * \
								cnt_w;
...
int			draw_minimap(t_info *info)
{
	...
	if (bonus.mini.w > 0 && bonus.mini.h > 0 && \
		bonus.mini.w < info->win.x && bonus.mini.h < info->win.y)
	{
		draw_map(info, &bonus);
